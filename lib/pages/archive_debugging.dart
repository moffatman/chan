...
class WrappedArchive extends ImageboardSite {
  ...
  Future<List<ImageboardBoard>> getBoards({required RequestPriority priority, CancelToken? cancelToken}) {
    return archive.getBoards(priority: priority, cancelToken: cancelToken);
  }
  ...
  Future<List<Thread>> getMoreCatalogImpl(String board, Thread after, {CatalogVariant? variant, required RequestPriority priority, CancelToken? cancelToken}) {
    return archive.getMoreCatalogImpl(board, after, variant: variant, priority: priority, cancelToken: cancelToken);
  }
  ...
  Future<Thread> getThreadImpl(ThreadIdentifier thread, {ThreadVariant? variant, required RequestPriority priority, CancelToken? cancelToken}) {
    return archive.getThread(thread, priority: priority, cancelToken: cancelToken);
  }

  @override
  Future<Thread> getThreadFromArchive(ThreadIdentifier thread, {Future<void> Function(Thread)? customValidator, required RequestPriority priority, CancelToken? cancelToken, String? archiveName}) {
    return archive.getThread(thread, priority: priority, cancelToken: cancelToken);
  }
  ...
  Future<ImageboardArchiveSearchResultPage> search(ImageboardArchiveSearchQuery query, {required int page, ImageboardArchiveSearchResultPage? lastResult, required RequestPriority priority, CancelToken? cancelToken}) {
    return archive.search(query, page: page, lastResult: lastResult, priority: priority, cancelToken: cancelToken);
  }
