...
class SiteHackerNews extends ImageboardSite {
		...
		switch (d['type']) {
			...
			case 'poll':
				return _HNStory(
					...
					time: DateTimeConversion.fromSecondsSinceEpoch(d['created_at_i'] as int),
	...
	Future<Thread?> _getThreadForCatalog(int id, {required RequestPriority priority, CancelToken? cancelToken}) async {
		...
		final op = Post(
			...
			text: text,
			name: d['by'] as String,
			time: DateTimeConversion.fromSecondsSinceEpoch(d['time'] as int),
			threadId: id,
			id: id,
			...
			posts: (data['hits'] as List).cast<Map>().map((hit) {
				...
				if (hit['comment_text'] != null) {
					return ImageboardArchiveSearchResult.post(Post(
						...
						time: DateTimeConversion.fromSecondsSinceEpoch(hit['created_at_i'] as int),
